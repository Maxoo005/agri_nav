...
AgriNav to projekt studencki, tworzony z myślą o gospodarstwie rodzinnym — nie komercyjny produkt od pierwszego dnia, tylko praktyczne narzędzie łączące naukę z realną potrzebą. Prowadzenie równoległe z centymetrową dokładnością istnieje na rynku od dawna, ale w cenie trudnej do uzasadnienia dla mniejszego gospodarstwa. Zamiast kupować gotowy system za kilka-kilkanaście tysięcy złotych, powstała własna aplikacja robiąca to samo na telefonie — niższym kosztem, i przy okazji jako sposób na poszerzenie własnej wiedzy z zakresu geodezji, systemów GNSS, programowania na styku Flutter i C++, oraz pracy z rzeczywistym sprzętem pomiarowym.

Sposób, w jaki projekt był rozwijany, widać wyraźnie w historii commitów i w samym kodzie: to nie jest praca zaplanowana od początku do końca na papierze, tylko seria iteracji testowanych bezpośrednio w polu — stąd nierówny rytm commitów (dni intensywnej pracy przeplatane przerwami) i stąd konkretne poprawki wynikające z rzeczywistych obserwacji w kabinie ciągnika, nie z teorii. Charakterystyczne jest podejście do weryfikacji numerycznej przed przyjęciem rozwiązania — najlepszym przykładem jest odrzucenie tańszego, ale mylącego kryterium oceny kierunku ścieżek na rzecz dokładnej oceny przez operacje boolowskie, mimo wyższego kosztu obliczeniowego (rozdział 6). Kod jest też nietypowo gęsto skomentowany z naciskiem na *dlaczego*, nie tylko *co* — świadoma decyzja, żeby uzasadnienia nietrywialnych wyborów (konwencje znaków, progi, kompromisy) zostały zapisane obok kodu, a nie tylko w pamięci autora.

### Co dalej — w stronę inteligentnego pola

AgriNav w obecnej formie (precyzyjna nawigacja, generowanie ścieżek, historia prac) to dopiero pierwszy etap większego celu. Docelowo projekt ma stać się fundamentem pod inteligentne zarządzanie gospodarstwem — systemem, który nie tylko prowadzi maszynę po polu, ale aktywnie pomaga podejmować lepsze decyzje agrotechniczne, maksymalizować plony i dbać o ziemię w sposób świadomy i mierzalny.

Kluczowe kierunki dalszego rozwoju:

- **Własna, precyzyjna aplikacja pogodowa** — zamiast polegać na ogólnych prognozach obejmujących duży obszar, celem jest zbudowanie źródła danych pogodowych dopasowanego dokładnie do lokalizacji konkretnych pól, wspierającego decyzje o terminach zabiegów (opryski, nawożenie, żniwa) z większą pewnością niż standardowe aplikacje pogodowe.
- **Spis i mapowanie terenu** — prowadzenie ustrukturyzowanej ewidencji areału: jakie pola, jakie uprawy, jaki rozkład gruntów w gospodarstwie, budując z czasem pełny, cyfrowy obraz całego gospodarstwa zamiast rozproszonej wiedzy „w głowie”.
- **Analiza potrzeb gleby** — integracja wyników badań gleby (zapotrzebowanie na składniki odżywcze, pH, struktura) z konkretnymi polami, jako podstawa do świadomego, celowanego nawożenia zamiast uniwersalnego podejścia do całego gospodarstwa.
- **Planowanie zabiegów wspierane sztuczną inteligencją** — docelowo połączenie danych pogodowych, glebowych i historii prac w system rekomendujący optymalny termin i sposób wykonania zabiegu, tak by każda interwencja (oprysk, nawożenie, siew) była maksymalnie trafiona — mniej strat, lepsze wykorzystanie zasobów, wyższa wydajność pracy.

To ambitny, wieloletni kierunek — obecna wersja AgriNav (precyzyjna nawigacja RTK) jest jego pierwszym, praktycznym filarem: bez dokładnej, cyfrowej wiedzy o tym, gdzie i jak dokładnie pracuje maszyna, trudno byłoby budować cokolwiek bardziej inteligentnego na tym fundamencie. Kolejne etapy będą rozwijane w miarę potrzeb gospodarstwa i dostępnego czasu.
